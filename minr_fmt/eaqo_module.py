...
class EAQOTrainingLightningModule(TrainingLightningModule):
    ...
    def _call_ssq_model(self, batch, return_diagnostics: bool = False):
        ...
        out = super()._call_ssq_model(batch, return_diagnostics=return_diagnostics or enabled)
        if not enabled:
            return out
        density = out["density"]
    ...
    def _eaqo_view_ambiguity(diag, density):
        feat = diag.get("sample_features") if isinstance(diag, dict) else None
        a = diag.get("A") if isinstance(diag, dict) else None
        valid = diag.get("query_view_valid") if isinstance(diag, dict) else None
        ...
        mean = (strength * vf).sum(dim=1) / vf.sum(dim=1).clamp_min(1.0)
        var = ((strength - mean[:, None]).square() * vf).sum(dim=1) / vf.sum(dim=1).clamp_min(1.0)
        return var / mean.abs().clamp_min(1.0e-6)
