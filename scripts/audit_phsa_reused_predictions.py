    ...
    for model in args.models:
        ...
        for index, sample_id in enumerate(expected_ids, start=1):
            ...
            with np.load(path, allow_pickle=False) as payload:
                if "proposal_indices" not in payload.files:
                    raise RuntimeError(f"{path} has no proposal_indices")
                ...
                if "gt" not in payload.files:
                    raise RuntimeError(f"{path} has no cached GT shape")
                shape = tuple(int(value) for value in payload["gt"].shape)
                ...
                if "proposal_sha256" not in payload.files:
                    raise RuntimeError(f"{path} has no proposal_sha256")
                ...
                if "pred" not in payload.files or payload["pred"].shape != shape:
                    raise RuntimeError(f"{path} prediction/GT shapes do not match")
            if index % 50 == 0:
