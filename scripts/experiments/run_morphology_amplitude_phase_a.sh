...
: "${PHASE_A_CKPT:?Set PHASE_A_CKPT to epoch=35-val_dice=0.7507.ckpt}"
...
PATIENCE="${PATIENCE:-3}"
LR="${LR:-0.00001}"
SUPPORT_LR="${SUPPORT_LR:-0.0001}"
VAL_EVERY="${VAL_EVERY:-1}"
RUN_REFERENCE_VALIDATE="${RUN_REFERENCE_VALIDATE:-1}"
RUN_FULL_EVAL="${RUN_FULL_EVAL:-1}"
EVAL_MAX_SAMPLES="${EVAL_MAX_SAMPLES:-100}"
CHUNK_SIZE="${CHUNK_SIZE:-32768}"
THRESHOLD="${THRESHOLD:-0.5}"
RUN_ROOT="${RUN_ROOT:-outputs/morphology_amplitude/unified_phase_a_seed${SEED}}"
...
COMMON=(
  ...
  "++model.ssq_fmt.view_complementary.sample_level_hypotheses.count=4096"
  "++model.ssq_fmt.view_complementary.sample_level_hypotheses.seed=42"
  ...
  "optim.lr=${LR}"
  "model.ssq_fmt.view_complementary.factorized_reconstruction.support_lr=${SUPPORT_LR}"
...
evaluate_checkpoint() {
  ...
  local eval_dir="${RUN_ROOT}/${name}/full_volume_val"
...
run_variant() {
  local name="$1" enabled="$2" compose="$3" support_w="$4" amplitude_w="$5"
  local out="${RUN_ROOT}/${name}"
  local last="${out}/checkpoints/last.ckpt"
  mkdir -p "$out"
  log "Training ${name}: enabled=${enabled}, compose=${compose}, support=${support_w}, amplitude=${amplitude_w}"
  local cmd=(
    ...
    "model.ssq_fmt.view_complementary.factorized_reconstruction.component_weight=0.0"
  )
  if [[ -f "$last" ]]; then
    cmd+=(
      "model.finetune.init_from_ckpt=null"
      "ckpt_path=$(realpath "$last")"
...
mkdir -p "$RUN_ROOT"
log "Reproduced Phase-A checkpoint: ${PHASE_A_CKPT}"

if [[ "$RUN_REFERENCE_VALIDATE" == "1" ]]; then
  log "Verifying sampled-query reference before continuation"
  uv run python scripts/train_morphology_amplitude.py validate \
    "${COMMON[@]}" \
    "paths.output_dir=${RUN_ROOT}/phase_a_reference_validate" \
    model.ssq_fmt.view_complementary.factorized_reconstruction.enabled=false
fi
...
# No Phase B/C is run. These are matched, single-stage continuations of the same
# reproduced shared reconstruction function.
run_variant scalar_control false true 0.0 0.0
run_variant support_aux true false 0.10 0.0
run_variant factorized_core true true 0.10 0.25

if [[ "$RUN_FULL_EVAL" == "1" ]]; then
  uv run python scripts/analysis/compare_morphology_amplitude.py --run_root "$RUN_ROOT"
fi
log "Screening complete. Results: ${RUN_ROOT}"
