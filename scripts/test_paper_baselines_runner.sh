...
run_test() {
  ...
  local cmd=(uv run python "$ROOT_DIR/scripts/eval_full_volume_fmt_simgen.py"
    ...
    --split test --threshold "$threshold" --save_dir "$save_dir" --device auto --save_predictions)
  if [[ -n "$ckpt" ]]; then
    cmd+=(--ckpt_path "$ckpt")
  fi
  cmd+=("$@")
...
for model in "${TRAINED[@]}"; do
  ...
  if [[ "$model" == two_stage_deepfmt_fixed ]]; then
    ...
    run_test "$model" "$config" "$ckpt" 0.5 \
...
for model in "${ANALYTIC[@]}"; do
  run_test "$model" "$model" "" "${ANALYTIC_THRESHOLD[$model]}"
done
