"""Training entrypoint for SSQ-FMT and comparison baselines.
...
from minr_fmt.benchmark import validate_baseline_protocol, write_baseline_manifest
...
def _resolve_eval_checkpoint(cfg: DictConfig, task: str, ckpt_path):
    if task not in {"validate", "test"}:
        return ckpt_path
    if ckpt_path is not None and str(ckpt_path) not in {"", "null"}:
        return ckpt_path

    last_ckpt = Path(str(cfg.paths.checkpoint_dir)) / "last.ckpt"
    if last_ckpt.exists():
        rank_zero_log(logger, "info", f"ckpt_path not set; using {last_ckpt}")
        return str(last_ckpt)

    allow_untrained = bool(cfg.get("allow_untrained_eval", False))
    if allow_untrained:
        rank_zero_log(
            logger,
            "warning",
            "No checkpoint was found; allow_untrained_eval=true, so evaluation will use random weights.",
        )
        return None
    raise FileNotFoundError(
        "Validation/test requires a trained checkpoint. Set ckpt_path, place last.ckpt in the "
        "configured checkpoint directory, or explicitly set allow_untrained_eval=true for a smoke test."
    )
...
def run(cfg: DictConfig) -> Optional[float]:
    ...
    manifest_path = write_baseline_manifest(cfg, cfg.paths.output_dir)
    rank_zero_log(
        logger,
        "info",
        f"Benchmark method: {spec.display_name} [{spec.fidelity}]; manifest={manifest_path}",
    )
    ...
    ckpt_path = _resolve_eval_checkpoint(cfg, task, cfg.get("ckpt_path"))
    ...
    if task == "fit":
        ...
        metric = trainer.callback_metrics.get("val_full_dice", trainer.callback_metrics.get("val_dice"))
